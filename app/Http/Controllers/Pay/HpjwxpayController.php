...
class HpjwxpayController extends PayController
{
    ...
    public function gateway($payway, $oid)
    {
        ...
        $data = [
            ...
            'type'   => 'WAP',//固定值"WAP"
            'wap_url'   => site_url(),//网站域名，必填
            'wap_name'   => site_url(),//网站域名，或者名字，必填，长度32或以内
        ...
        try {
            Log::channel('single')->debug('信息', ['xx' => 'xx']);
    ...
    public function http_post($url,$data){
        if(!function_exists('curl_init')){
            throw new RuleValidationException('php未安装curl组件',500);
        }
        ...
        if($httpStatusCode!=200){
            throw new RuleValidationException("invalid httpstatus:{$httpStatusCode} ,response:$response,detail_error:".$error,$httpStatusCode);
        }
        return $response;
    }
